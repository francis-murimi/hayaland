...
/// Find parties whose primary location falls within a given radius.
pub async fn nearby_parties(
    ...
    if query.radius_km.is_none() || query.latitude.is_none() || query.longitude.is_none() {
        return Err(ApiError::Validation(
            "nearby search requires lat, lng and radiusKm".to_string(),
        ));
    }
