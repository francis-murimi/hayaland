...
pub struct FakeDealRepo {
    pub deals: Mutex<HashMap<Uuid, domain::entities::Deal>>,
    pub participations: Mutex<Vec<domain::entities::DealParticipation>>,
    pub terms: Mutex<Vec<domain::entities::Term>>,
    pub value_distributions: Mutex<HashMap<Uuid, domain::entities::ValueDistribution>>,
    pub history: Mutex<Vec<(Uuid, String, Option<Uuid>, Option<serde_json::Value>)>>,
...
impl WalletRepository for FakeWalletRepo {
    ...
    async fn record_multi_party_transaction(
        ...
        for wallet in wallets {
            guard.insert(wallet.party_id, wallet.clone());
        }
        drop(guard);
        self.transactions.lock().unwrap().push(transaction.clone());
        Ok(())
...
impl CatalogRepository for FakeCatalogRepository {
    ...
    async fn list_resources(
        ...
        let mut items: Vec<Resource> = all
            .values()
    ...
    async fn count_resources_for_party(&self, party_id: Uuid) -> Result<i64, DomainError> {
        Ok(self
            ...
            .unwrap()
            .values()
            .filter(|r| r.supplier_party_id == party_id && r.is_active && !r.platform_hidden)
            .count() as i64)
    }
    ...
    async fn update_need(&self, need: &Need) -> Result<(), DomainError> {
        ...
        if needs.contains_key(&need.id) {
            ...
            Err(DomainError::NeedNotFound)
        }
    }
    async fn delete_need(&self, id: Uuid) -> Result<(), DomainError> {
        self.needs.lock().unwrap().remove(&id);
        Ok(())
    }
    ...
    async fn list_needs(
        ...
        let all = self.needs.lock().unwrap();
        let mut items: Vec<Need> = all
            .values()
    ...
    async fn count_needs_for_party(&self, party_id: Uuid) -> Result<i64, DomainError> {
        Ok(self
            ...
            .unwrap()
            .values()
            .filter(|n| n.consumer_party_id == party_id && n.is_active && !n.platform_hidden)
            .count() as i64)
    }
    ...
    async fn update_enhancement(&self, enhancement: &Enhancement) -> Result<(), DomainError> {
        ...
        if enhancements.contains_key(&enhancement.id) {
            ...
            Err(DomainError::EnhancementNotFound)
        }
    }
    ...
    async fn list_enhancements(
        ...
        let all = self.enhancements.lock().unwrap();
        let mut items: Vec<Enhancement> = all
            .values()
    ...
    async fn count_enhancements_for_party(&self, party_id: Uuid) -> Result<i64, DomainError> {
        Ok(self
            ...
            .unwrap()
            .values()
            .filter(|e| e.enhancer_party_id == party_id && e.is_active && !e.platform_hidden)
            .count() as i64)
    }
    ...
    async fn update_resource_admin_flags(
        ...
        if let Some(r) = resources.get_mut(&id) {
            ...
            if let Some(v) = flags.platform_featured {
                r.platform_featured = v;
            }
            if let Some(v) = flags.admin_notes {
                r.admin_notes = Some(v);
            }
            if let Some(v) = flags.admin_reviewed_by {
                r.admin_reviewed_by = Some(v);
                r.admin_reviewed_at = Some(OffsetDateTime::now_utc());
            }
            Ok(())
        } else {
            Err(DomainError::ResourceNotFound)
    ...
    async fn update_need_admin_flags(&self, id: Uuid, flags: AdminFlags) -> Result<(), DomainError> {
        ...
        if let Some(n) = needs.get_mut(&id) {
            ...
            if let Some(v) = flags.platform_featured {
                n.platform_featured = v;
            }
            if let Some(v) = flags.admin_notes {
                n.admin_notes = Some(v);
            }
            if let Some(v) = flags.admin_reviewed_by {
                n.admin_reviewed_by = Some(v);
                n.admin_reviewed_at = Some(OffsetDateTime::now_utc());
            }
            Ok(())
        } else {
            Err(DomainError::NeedNotFound)
    ...
    async fn update_enhancement_admin_flags(
        ...
        let mut enhancements = self.enhancements.lock().unwrap();
        if let Some(e) = enhancements.get_mut(&id) {
            if let Some(v) = flags.platform_hidden {
                e.platform_hidden = v;
            }
            if let Some(v) = flags.platform_featured {
                e.platform_featured = v;
            }
            if let Some(v) = flags.admin_notes {
                e.admin_notes = Some(v);
            }
            if let Some(v) = flags.admin_reviewed_by {
                e.admin_reviewed_by = Some(v);
                e.admin_reviewed_at = Some(OffsetDateTime::now_utc());
            }
            Ok(())
        } else {
            Err(DomainError::EnhancementNotFound)
...
pub struct TestFixtures {
    ...
    pub queue: Arc<FakeEmailQueue>,
    pub deal_repo: Arc<FakeDealRepo>,
    pub party_repo: Arc<FakePartyRepo>,
    pub wallet_repo: Arc<FakeWalletRepo>,
    pub milestone_repo: Arc<FakeMilestoneRepo>,
    pub catalog_repo: Arc<FakeCatalogRepository>,
}
...
pub fn test_fixtures() -> TestFixtures {
    ...
    let catalog_repo: Arc<FakeCatalogRepository> = Arc::new(FakeCatalogRepository::default());
    let db_pool =
...
pub fn extract_token_for_email(queue: &FakeEmailQueue, email: &str) -> String {
    extract_token_for_email_with_path(queue, email, "/auth/verify-email?token=")
}

pub fn extract_reset_token_for_email(queue: &FakeEmailQueue, email: &str) -> String {
...
pub async fn login(fixtures: &TestFixtures, email: &str) -> String {
